...
class AnalysisType(Enum):
    ...
    BENEFITS = "Benefits"  # NEW: Benefits analysis type
...
class AppConfig:
    ...
    # NEW: Benefits-specific column mappings
    # Fill these in when your dataset has the benefits columns ready
    # Format: (Display Name, Column Suffix)
    BENEFITS_COLS = [
        # Example entries (replace with actual column names):
        # ("Social Security Benefits Reform", "Social Security Reform"),
        # ("Medicare Benefits Reform", "Medicare Reform"),
        # ("Medicaid Benefits Reform", "Medicaid Reform"),
        # ("SNAP Benefits Reform", "SNAP Reform"),
        # ("Child Care Benefits Reform", "Child Care Reform"),
        # Add more as needed...
    ]
    ...
    # NEW: Placeholder column names for Benefits analysis
    # Update these with actual column names when available
    BASELINE_BENEFITS_COLUMN = "Baseline Total Benefits"  # Replace with actual column name
    TOTAL_BENEFITS_CHANGE_COLUMN = "Total Change in Benefits"  # Replace with actual column name
    PCT_BENEFITS_CHANGE_COLUMN = "Percentage Change in Benefits"  # Replace with actual column name
    BENEFITS_CHANGE_PREFIX = "Change in Benefits after"  # Replace with actual prefix
...
class TaxAnalysisEngine:
    ...
    def get_reform_impacts(self, household_data: pd.Series) -> List[ReformImpact]:
        ...
        # NEW: Use appropriate column list based on analysis type
        if self.analysis_type == AnalysisType.BENEFITS:
            reform_cols = AppConfig.BENEFITS_COLS
        else:
            reform_cols = AppConfig.REFORM_COLS
        
        for display_name, col_name in reform_cols:
            try:
    ...
    def get_chart_title(self) -> str:
        ...
        titles = {
            ...
            AnalysisType.BENEFITS: "Total Benefits"  # NEW
        }
    ...
    def get_baseline_info(self, profile: HouseholdProfile, household_data: pd.Series) -> Tuple[float, str]:
        ...
        mapping = {
            ...
            AnalysisType.BENEFITS: (household_data.get(AppConfig.BASELINE_BENEFITS_COLUMN, 0), "Total Benefits")  # NEW
        }
        return mapping[self.analysis_type]
    
    def get_change_info(self, household_data: pd.Series) -> Tuple[float, float, str, str, str, float]:
        """
        Get change information for impact summary. Helps to calculate post-reform value, too.
        
        Returns:
            Tuple[float, float, str, str, str, float]: (change_value, pct_change, change_label, final_label, color, final_value)
        """
        mapping = {
            ...
            AnalysisType.BENEFITS: (  # NEW
                household_data.get(AppConfig.TOTAL_BENEFITS_CHANGE_COLUMN, 0),
                household_data.get(AppConfig.PCT_BENEFITS_CHANGE_COLUMN, 0),
                "Benefits Change",
                household_data.get(AppConfig.BASELINE_BENEFITS_COLUMN, 0), "Reformed Benefits"
            )
        }
        ...
        if self.analysis_type in [AnalysisType.NET_INCOME, AnalysisType.BENEFITS]:
            color = "green" if change_value > 0 else "red"  # Income/Benefits increase is good
        else:
    ...
    def _get_column_prefix(self) -> str:
        ...
        prefixes = {
            ...
            AnalysisType.BENEFITS: AppConfig.BENEFITS_CHANGE_PREFIX  # NEW
        }
...
class VisualizationRenderer:
    ...
    def _build_additional_taxes_content(self, profile: HouseholdProfile, household_data: pd.Series) -> str:
        ...
        baseline_benefits = household_data.get(AppConfig.BASELINE_BENEFITS_COLUMN, 0)
        ...
        tax_mappings = {
            ...
            AnalysisType.BENEFITS: [  # NEW
                f"Federal Tax Liability: ${profile.baseline_federal_tax:,.0f}",
                f"State Taxes: ${state_tax:,.0f}" if state_tax > 0 else None,
                # Add more benefit-related items here as needed
            ]
        }
    ...
    def _create_waterfall_figure(self, waterfall_data: List[Tuple], chart_title: str, 
        ...
        # NEW: Adjust colors based on analysis type
        if self.analysis_engine.analysis_type in [AnalysisType.NET_INCOME, AnalysisType.BENEFITS]:
            increasing_color = "green"  # Positive change is good
            decreasing_color = "red"    # Negative change is bad
        else:
            increasing_color = "red"    # Tax increase is bad
            decreasing_color = "green"  # Tax decrease is good
    ...
    def render_analysis_info_card(self) -> None:
        ...
        # NEW: Get appropriate reform list based on analysis type
        if self.analysis_engine.analysis_type == AnalysisType.BENEFITS:
            reform_cols = AppConfig.BENEFITS_COLS
            if not reform_cols:
                st.info("📋 **Analysis Scope:** Benefits analysis columns are not yet configured. Please update the BENEFITS_COLS list with actual column names.")
                return
        else:
            reform_cols = AppConfig.REFORM_COLS
        
        # Format reform names list
        reform_names = [display_name for display_name, _ in reform_cols]
        if reform_names:
            reforms_text = ", ".join(reform_names[:-1]) + f", and {reform_names[-1]}" if len(reform_names) > 1 else reform_names[0]
        else:
            reforms_text = "no specific reforms"
...
class HouseholdDashboard:
    ...
    def _render_analysis_type_selector(self) -> AnalysisType:
        ...
        # NEW: Include Benefits in the options
        analysis_type = st.sidebar.radio(
            "Select what to analyze:",
            [
                AnalysisType.FEDERAL_TAXES.value, 
                AnalysisType.STATE_TAXES.value, 
                AnalysisType.NET_INCOME.value,
                AnalysisType.BENEFITS.value  # NEW
            ],
            index=0
        )
        ...
        type_mapping = {
            ...
            AnalysisType.BENEFITS.value: AnalysisType.BENEFITS  # NEW
        }
